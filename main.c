...
int main(void)
{
    ...
    mpz_init(Sm4tSm4);
    mpz_init(Sm2t2);
    mpz_set_str(Sbig, "109790245", 10);

    for(;;mpz_add_ui(Sbig, Sbig, 1)) {
        mpz_set_ui(pns, 8);
        mpz_set_ui(sum, 1);
        mpz_sub_ui(Sm2, Sbig, 2);
        // Skip when there is a parametric solution: S = 3n+2
        ...
        for(;mpz_cmp(n, big); mpz_add_ui(n, n, 1)) {
            // 8*pns = n(S-2)*8+8+pns*8
            mpz_add(tmp1, tmp1, Sm2t8);
            mpz_add(pns, tmp1, pns);

            // sum = sum+pns*8*(S-2)
            mpz_mul(tmp2, pns, Sm2);
